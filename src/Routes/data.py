...
async def process_endpoint(project_id: str,processing_request: ProcessingRequest,request:Request):
    ...
    project_files_ids= {}
    if processing_request.file_id:
        asset_record= await asset_model.get_asset_record(
            asset_project_id=project.id,
            asset_name=processing_request.file_id
        ) 

        if asset_record is None:
             return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={"is_valid":False , "result_signal":ResponseSignal.FILE_ID_ERROR.value} )  
    ...
    if len(project_files_ids)==0:  
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={
                "result_signal":ResponseSignal.NO_FILES_ERROR.value}
                )
